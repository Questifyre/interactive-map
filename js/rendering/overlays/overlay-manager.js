...
export const toggleOverlay = async (overlayName) => {
    ...
    if (overlay.type === 'static') {
        if (activeStaticOverlays > 0 && !isStaticListening) {
            window.addEventListener('staticOverlaysUpdatedEvent', handleStaticOverlaysUpdated);
            isStaticListening = true;
        } else if (activeStaticOverlays === 0 && isStaticListening) {
            window.removeEventListener('staticOverlaysUpdatedEvent', handleStaticOverlaysUpdated);
            isRenderingFilter = false;
            isStaticListening = false;
        }
    } else {
        if (overlay.state && !isDynamicListening) {
            window.addEventListener('dynamicOverlaysUpdated', handleDynamicOverlays);
            isDynamicListening = true;
        } else if (!overlay.state && isDynamicListening) {
            window.removeEventListener('dynamicOverlaysUpdated', handleDynamicOverlays);
    ...
    try {
        ...
        overlay.state = !overlay.state;
        totalActiveOverlays += overlay.state ? 1 : -1;
        throw error;
    } finally {
