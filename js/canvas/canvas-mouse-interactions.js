...
import { CURSOR_GRAB, CURSOR_GRABBING, tryUpdateRegionSound } from './canvas-interactions.js';
...
export const setupMouseSystem = (canvas) => {
    ...
    const handleZoom = (zoomFactor, centerX, centerY, isTouch = false) => {
        const scaledZoom = Math.pow(zoomFactor, 1); // Sensitivity is 1 for PC
        const constrainedScale = Math.min(Math.max(CANVAS_TRANSFORM.scale * scaledZoom, MIN_ZOOM_SCALE), MAX_ZOOM_SCALE);
        const scaleRatio = constrainedScale / CANVAS_TRANSFORM.scale;
        CANVAS_TRANSFORM.offsetX = centerX - (centerX - CANVAS_TRANSFORM.offsetX) * scaleRatio;
        CANVAS_TRANSFORM.offsetY = centerY - (centerY - CANVAS_TRANSFORM.offsetY) * scaleRatio;
